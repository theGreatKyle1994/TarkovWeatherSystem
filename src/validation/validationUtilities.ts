...
function checkSeasonDB(dbSeason: SeasonDB, logger: ILogger): void {
    ...
    if (
        !Object.values(SeasonName).includes(dbSeason.seasonName as SeasonName)
    ) {
        isError = true;
        logger.error(
...
function validateConfigKeys<ConfigType, ModelType>(
    ...
    for (let key in config) {
        ...
        if (!Object.hasOwn(model as Object, key)) {
            isError = true;
            logger.error(
                `[TWS] Invalid key: "${key}" found in ${fileName}.json.`
            );
            delete config[key];
        }
        ...
        if (typeof model[key as string] !== typeof config[key]) {
            isError = true;
            logger.error(
                `[TWS] Key: "${key}" has an incorrect type of: ${typeof config[
                    key
                ]}.`
            );
